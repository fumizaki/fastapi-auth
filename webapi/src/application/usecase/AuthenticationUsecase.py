...
class AuthenticationUsecase:
    ...
    async def v1_signup_exec(self, param: SignUpSchema) -> AccountEntity:
        try:
            account = AccountEntity(
                email=AccountEmail(param.email),
                password=AccountPassword(param.password)
            )
            account_in_db = await self.uow.account_repository.insert(account)
            await self.uow.rdb.commit()
            return account_in_db
        
        finally:
            await self.uow.rdb.close()